...
const PostsPreview = ({ post }: { post: PostPreview|undefined }) => {
        ...
        <div className="flex flex-col mt-20 h-full px-0 py-30 content-center items-center">{/*card*/}
            ...
            <div className="overflow-hidden h-full rounded-lg">{/*card wrapper для эффекта hover-scale картинки*/}
                <img className="transform hover:scale-150 ease-in-out duration-700 w-full h-fit rounded-lg"
                    src={post && post.path ? post.path : 'img/postspage/cloudsWIDE.webp'} alt="Post image"/>{/*card-img*/}
            </div>
            ...
            <div
                ...
                <h5 className="text-center text-blue-950 font-semibold mt-4">
                    {post ? <Link
                        ...
                        href={`/posts/${post.id}`}>{post.title?.slice(0, 45)}...</Link> : <p>...Loading...</p>
                    }
                </h5>
                <p className="text-justify text-blue-950 px-2">&nbsp;{post ? post.preview : 'Text is loading...'}...</p>

                <p className="text-end text-blue-950 italic pr-2">Опубликовано:&nbsp;
                    {post ? post.createdAt?.toLocaleDateString('ru-RU', {
                        // weekday: 'short',
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
