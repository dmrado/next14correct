...
const CookieConsent = ({isAccepted}: { isAccepted: boolean }) => {
    ...
    return (
        <div
            className="fixed bottom-0 left-1/2 transform -translate-x-1/2 z-50 shadow-2xl
        w-11/12      // На мобильных: Занимаем 91.6% ширины экрана
        max-w-xl     // Максимальная ширина на всех экранах (xl = 36rem/576px)
        md:max-w-lg  // На средних экранах max 32rem/512px

        // Стиль:
        flex flex-col
        bg-gray-700 rounded-lg p-4 text-white text-sm text-left space-y-3">
            <div
                className="flex flex-col fixed bottom-0 text-center z-20">
                На этом сайте мы используем файлы cookie, чтобы улучшить ваш опыт использования и предоставить
                персонализированный контент. Некоторые из этих файлов cookie устанавливаются сторонними сервисами,
                такими
                как Google (Google является зарегистрированным товарным знаком Google LLC) и Яндекс (Яндекс является
                зарегистрированным товарным знаком Яндекса).
                ...
                относительно использования файлов cookie. Для получения более подробной информации о том, как мы
                обрабатываем ваши данные пожалуйста, обратитесь к нашей
